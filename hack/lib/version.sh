...
function cos::version::get_version_vars() {
  ...
  if [[ -n ${GIT_VERSION:-} ]] || GIT_VERSION="$(git rev-parse --abbrev-ref HEAD 2>/dev/null | sed -E 's/[^a-zA-Z0-9]+/-/g' 2>/dev/null)"; then
    # check if the HEAD is tagged.
    if git_tag=$(git tag -l --contains HEAD | head -n 1 2>/dev/null) && [[ -n ${TAG:-${DRONE_TAG:-${git_tag}}} ]]; then
      GIT_VERSION="${TAG:-${DRONE_TAG:-${git_tag}}}"
    fi
  fi
